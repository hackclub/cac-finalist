...
function getColor(weight) {
  if (weight === undefined) return "#ccc";
  const lightBlue = [204, 229, 255];
  const darkBlue = [0, 51, 153];
  const r = Math.round(lightBlue[0] + (darkBlue[0] - lightBlue[0]) * weight);
  const g = Math.round(lightBlue[1] + (darkBlue[1] - lightBlue[1]) * weight);
  const b = Math.round(lightBlue[2] + (darkBlue[2] - lightBlue[2]) * weight);
  return `rgb(${r},${g},${b})`;
}
...
export default function CongressionalMap() {
  ...
  const onEachFeature = (feature, layer) => {
    ...
    // Key includes both state and district codes padded as needed
    const districtKey = `${stateFP}${districtNum < 10 ? "0" + districtNum : districtNum}`;

    const weight = districtWeights[districtKey];

    let tooltipText = `${stateName} ${districtStr}`;
    tooltipText += weight !== undefined ? ` — Participants: ${weight}` : ` — No Participants`;
